...
$this->load->view('parts/nav');
 ?>
	...
	<div class="modal fade" id="importmodal">
		...
		<form method="post" id="import_csv" action="<?php echo site_url();?>/dataprodusen/ExcelDataAdd" enctype="multipart/form-data" method="post">
...
echo form_open_multipart('dataprodusen/input','class="form-horizontal" role="form" id="fpelanggan"');
	...
	<div class="modal fade" id="tambahpelanggan">
			  	<div class="modal-dialog">
			    	<div class="modal-content">
			      		<div class="modal-header">
			        		<button type="button" class="close" data-dismiss="modal"><span aria-hidden="true">&times;</span><span class="sr-only">Close</span></button>
			        		<h4 class="modal-title">Tambah Produsen <br><small>Silahkan isi data pelanggan di form ini.</small></h4>
			      		</div>
			      		<div class="modal-body">
			        		...
			        		<form class="form-horizontal no-margin" id="type-constraint" data-validate="parsley" novalidate>
											<div class="form-group">
												...
												<div class="col-lg-9">
													<input type="text" class="form-control input-sm" placeholder="Nama Produsen" name="nama_produsen">
												</div><!-- /.col -->
											</div><!-- /form-group -->
											<div class="form-group">
...
<?php 
echo "<h5>".$this->session->flashdata('error')."</h5>";
echo form_open_multipart('dataprodusen/update','class="form-horizontal" role="form" id="fpelangganupdate"');
			  	...
			  	<div class="modal-dialog">
			    	<div class="modal-content">
			      		<div class="modal-header">
							...
							<input type="hidden" name="kode_produsen" id="kode_produsen">			        		
			      		</div>
			      		<div class="modal-body">
			        		...
			        		<form class="form-horizontal no-margin" id="type-constraint" data-validate="parsley" novalidate>
											...
											<div class="radio inline-block">
												<div class="custom-radio m-right-xs">
													<input type="radio" id="rd_stspro" name="sts" value="Aktif">
													<label for="rd_stspro"></label>
												</div>
												<div class="inline-block vertical-top">Aktif </div>
											</div>&nbsp;
											<div class="radio inline-block">
												<div class="custom-radio m-right-xs">
													<input type="radio" id="rd_tidakpro" name="sts" value="Tidak Aktif">
													<label for="rd_tidakpro"></label>
												</div>
												<div class="inline-block vertical-top"> Tidak Aktif</div>
											</div>
										</div><!-- /.col -->
...
<div class="main-container">
				...
				<div class="padding-md">
					<ul class="breadcrumb">
						...
						<button class="btn btn-primary" data-toggle="modal" data-target="#tambahpelanggan"><i class="fa fa-plus-square-o m-right-xs" aria-hidden="true"></i>Tambah Produsen</button></span>
					</ul>
					...
					<table class="table table-striped" id="tablepelanggan">
						...
						<tbody>
							<?php 
		$i = 1;
		foreach ($data_produsen as $p) {
			echo "<tr>";
			echo "<td>".$i."</td>";
			echo "<td>".$p['kode_produsen']."</td>";
			echo "<td>".$p['nama']."</td>";
			echo "<td>".$p['alamat']."</td>";
			echo "<td>".$p['email']."</td>";
			echo "<td>".$p['status']."</td>";
			echo "<td>".$p['keterangan']."</td>";
			echo "<td>".anchor('dataprodusen/hapus/'.$p['kode_produsen'],'<input type="button"  class="btn btn-danger hapus-pelanggan" value= "Hapus"> | ');
...
$this->load->view('parts/footer');
  ...
  <script>
	 	...
	 	var kode_produsen = $(this).closest('tr').find('td:eq(1)').text();
	 	...
	 	var sts = $(this).closest('tr').find('td:eq(5)').text();
		var sts1 = "Aktif";
		var sts2 = "Tidak Aktif";
	 	...
	 	$('#email').val(email);
		 				 if (sts == sts1) {
		 	document.getElementById("rd_stspro").checked = true;
		 }
		 	else{
			document.getElementById("rd_tidakpro").checked = true;		 	
		}
    ...
    var url = $(this).closest('tr').find('a').attr('href');
      ...
      e.preventDefault();
      var kode_produsen = $(this).closest('tr').find('td:eq(1)').text();
      $('#message').html('Apakah data <br> '+kode_produsen+' </b> akan dihapus?');
      $('#message').dialog('open');
     })
  	   	 });
